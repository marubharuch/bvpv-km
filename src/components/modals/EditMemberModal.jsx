...
import { useMemberForm, EMPTY_FORM } from "../../hooks/useMemberForm";
      ...
      <div>
        ...
        <div className="flex gap-2">
          <input type="tel" inputMode="numeric" value={form.countryCode || "+91"}
            ...
            maxLength={5} style={{ fontSize:14,border:"2px solid #f0e6e6",background:"#fff",color:"#3D0010",minWidth:70,textAlign:"center" }}
          ...
          {[{value:"Male",en:"Male",gu:"પુરુષ",emoji:"👨"},{value:"Female",en:"Female",gu:"સ્ત્રી",emoji:"👩"}].map(g => (
            <button key={g.value} type="button" onClick={() => update("gender", form.gender===g.value?"":g.value)}
              ...
              style={form.gender===g.value
                ...
                : {background:"#fff",borderColor:"#f0e6e6",color:"#9B6060"}}>
              <span className="block">{g.emoji} {g.en}</span><span className="block text-xs opacity-80">{g.gu}</span>
            </button>
          ))}
        </div>
      </div>
...
function TabEducation({ form, update }) {
  ...
  const handleStudentToggle = () => {
    ...
    if (isStudent) { ["educationType","standard","stream","medium","year","degree","specialization","collegeName","courseName","courseStage","exam"].forEach(f=>update(f,"")); }
  };
  return (
    <div className="space-y-4 px-4 py-4">
  ...
  return (
    <div className="space-y-5 px-4 py-4">
      ...
      <div><FieldLabel>🏆 Achievements</FieldLabel>
        <textarea value={form.achievements} onChange={e=>update("achievements",e.target.value)} placeholder="Awards, competitions..." rows={3}
      ...
      <div><FieldLabel>👤 About</FieldLabel>
        <textarea value={form.aboutMe} onChange={e=>update("aboutMe",e.target.value)} placeholder="Personality, goals..." rows={3}
          style={{fontSize:16,border:"2px solid #f0e6e6"}} className="w-full rounded-xl px-4 py-3 text-sm outline-none focus:border-[#C9A84C] resize-none" /></div>
    </div>
  );
}
...
function TabHonorary({ form, update }) {
  ...
  const toggleOrg = orgId => hasOrg(orgId) ? update("honoraryOrgs",orgs.filter(o=>o.orgId!==orgId)) : update("honoraryOrgs",[...orgs,{orgId,post:"",name:""}]);
            ...
            <div key={org.id}>
              ...
              {selected && entry && (
                <div className="mt-2 ml-3 space-y-2 pl-3 border-l-2" style={{borderColor:"#C9A84C"}}>
                  {org.askName && (
                    <div>
                      <p className="text-xs font-semibold mb-1" style={{color:"#7B1C2E"}}>સંસ્થાનું નામ</p>
                      <input type="text" value={entry.name||""} onChange={e=>updateOrg(org.id,"name",e.target.value)} placeholder="સંસ્થાનું નામ..."
  ...
  return (
    <div className="space-y-4 px-4 py-4">
      ...
      <div><FieldLabel>Additional Notes</FieldLabel>
        <textarea value={form.helpRequired} onChange={e=>update("helpRequired",e.target.value)} placeholder="Any specific help..." rows={3}
          style={{fontSize:16,border:"2px solid #f0e6e6"}} className="w-full rounded-xl px-4 py-3 text-sm outline-none focus:border-[#C9A84C] resize-none" /></div>
    </div>
  );
}
...
function ReviewScreen({ form, onEdit, onSave, saving, isAdding }) {
  const rows = [
    ...
    {label:"Status",value:form.maritalStatus||""},{label:"Student",value:form.isStudent?"Yes":"No"},
    {label:"Occupation",value:form.occupation},{label:"Education",value:form.educationType},
...
export default function EditMemberModal({ open, mode = "edit", member = null, familyId, onClose }) {
  const { form, errors, saving, updateField, validate, handleSave, isAdding } =
  ...
  const goNext = () => {
    if (isLastTab) { if (validate()) setShowReview(true); }
    else setActiveTab(TAB_IDS[currentTabIndex + 1]);
  };
  const goBack = () => {
  ...
  return (
    <>
      ...
      <div className="fixed left-0 right-0 top-0 z-[200] flex flex-col rounded-b-3xl"
        style={{ background:"#FDF6EC", maxHeight:"92vh", boxShadow:"0 8px 40px rgba(90,16,32,0.25)", overflow:"hidden", paddingTop:"env(safe-area-inset-top, 0px)" }}>
        ...
        {!showReview && (
          <div className="flex gap-3 px-4 py-3 flex-shrink-0" style={{borderTop:"1px solid #f0e6e6",paddingBottom:"calc(12px + env(safe-area-inset-bottom, 0px))"}}>
            {!isFirstTab && (
              <button onClick={goBack} className="flex-1 py-3.5 rounded-xl text-sm font-semibold border-2" style={{borderColor:"#f0e6e6",color:"#7B1C2E"}}>
                ← Back
              </button>
            )}
            <button onClick={goNext} className="py-3.5 rounded-xl text-sm font-bold text-white transition-all"
              style={{background:"#7B1C2E", flex:1}}>
