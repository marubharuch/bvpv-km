...
// ─────────────────────────────────────────────────────────────────────────────
// 8-step VSCode-style wizard. On Step 8 "Save", converts wizard state to
// the graph schema and writes to families/{familyId} via familyTreeDb.
// If user already has a family, loads existing tree for editing.
// ─────────────────────────────────────────────────────────────────────────────
...
import { createFamilyTree, getFamilyTree } from "../../db/familyTreeDb";
import { getUserFamilyPointer }             from "../../db/familyTreeDb";
import { TREE_STEPS }         from "../../constants/vanshConstants";
import { COLORS }             from "../../constants/app";
import {
  VscTitlebar, VscTabs, VscProgressBar, VscStatusBar, VscBottomBar,
...
export default function VanshVriksha() {
  ...
  // ── Load existing tree if user already has one ───────────────────────────
  useEffect(() => {
    if (!user?.uid) { setLoading(false); return; }

    getUserFamilyPointer(user.uid)
      .then(async pointer => {
        if (pointer?.familyId) {
          setFamilyId(pointer.familyId);
          const treeDoc = await getFamilyTree(pointer.familyId);
          if (treeDoc) tree.loadFromFamilyDoc(treeDoc);
        }
      })
      .catch(console.error)
  ...
  const handleNext = async () => {
    ...
    if (tree.step === TREE_STEPS.FULL_TREE) {
      ...
      setSaveErr("");
      try {
        const result = await createFamilyTree(user.uid, null, {
          self:        tree.self,
  ...
  if (loading) return (
    <div style={{ minHeight:"100dvh", display:"flex", alignItems:"center", justifyContent:"center", background:COLORS.bg }}>
      <div style={{ textAlign:"center" }}>
        ...
        <p style={{ fontSize:"0.82rem", color:COLORS.primary }}>Tree લોડ થઈ રહ્યું છે...</p>
      </div>
    </div>
  );
  ...
  if (saved) return (
    <div style={{ minHeight:"100dvh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:COLORS.bg, padding:24, textAlign:"center" }}>
      ...
      <p style={{ fontSize:"0.82rem", color:COLORS.textSecondary, marginBottom:4 }}>
        Family ID: <strong style={{ color:COLORS.primary, fontFamily:"monospace" }}>{familyId}</strong>
      </p>
      <p style={{ fontSize:"0.78rem", color:COLORS.textSecondary, marginBottom:24 }}>
        Family members ને invite કરવા Family ID share કરો.
      </p>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100dvh", overflow:"hidden", background:COLORS.bg, fontFamily:"'JetBrains Mono', monospace" }}>
      <VscTitlebar />
      <VscTabs currentStep={tree.step} onJump={tree.jumpTo} />
      <VscProgressBar currentStep={tree.step} totalSteps={TOTAL} />

      <div style={{ flex:1, overflowY:"auto", overflowX:"hidden", WebkitOverflowScrolling:"touch" }}>
        {tree.step === TREE_STEPS.SELF      && <SelfStep self={tree.self} onChange={tree.setSelf} />}
        {tree.step === TREE_STEPS.ANCESTORS && <AncestorsStep ancestors={tree.ancestors} onChange={tree.setAncestors} />}
        {tree.step === TREE_STEPS.DESCS     && <DescendantsStep descendants={tree.descendants} onChange={tree.setDescendants} />}
        {tree.step === TREE_STEPS.PREVIEW   && <PreviewStep self={tree.self} ancestors={tree.ancestors} descendants={tree.descendants} />}
        ...
        {tree.step === TREE_STEPS.COUSINS   && (
          <CousinsStep cousins={tree.cousins} onToggle={tree.toggleCousin} onAdd={tree.addCousin} />
        )}
      ...
      <VscBottomBar currentStep={tree.step} totalSteps={TOTAL} onBack={tree.goBack} onNext={handleNext} saving={saving} />
...
function showToast(msg) {
  ...
  t.style.cssText = `position:fixed;bottom:75px;left:50%;transform:translateX(-50%);background:#fff;border:1.5px solid #C9A84C;color:#7B1C2E;padding:7px 16px;border-radius:18px;font-size:0.75rem;z-index:300;pointer-events:none;white-space:nowrap;box-shadow:0 2px 10px rgba(90,16,32,0.15);font-family:'Lato',sans-serif;`;
  t.textContent = msg;
